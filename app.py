...
internships = st.number_input("Internships", min_value=0.0, step=1.0)

projects = st.number_input("Projects", min_value=0.0, step=1.0)

aptitude = st.number_input("Aptitude Score", min_value=0.0, max_value=100.0, step=0.1)

communication = st.slider("Communication Skills (1-10)", 1.0, 10.0, step=0.1)

training = st.selectbox("Placement Training", ["No", "Yes"])

# 🔹 Convert training to float
training_val = 1.0 if training == "Yes" else 0.0
